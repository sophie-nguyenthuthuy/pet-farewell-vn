...
export function BookingWizard({
  ...
  function validateCurrent(): boolean {
    ...
    if (step === 0) {
      if (!state.pet.name) stepErrors['pet.name'] = 'Vui lòng nhập tên bé';
      if (!state.pet.sizeBand) stepErrors['pet.sizeBand'] = 'Vui lòng chọn cân nặng';
    }
    if (step === 1) {
      if (!state.service.serviceId) stepErrors['service.serviceId'] = 'Vui lòng chọn dịch vụ';
    }
    if (step === 2) {
      if (state.pickup.pickupRequired && !state.pickup.pickupAddress) {
        stepErrors['pickup.pickupAddress'] = 'Vui lòng nhập địa chỉ đón';
      }
      if (!state.pickup.scheduledFor) {
        stepErrors['pickup.scheduledFor'] = 'Vui lòng chọn thời gian';
      }
    }
    if (step === 3) {
      if (!state.contact.fullName) stepErrors['contact.fullName'] = 'Vui lòng nhập họ tên';
      if (!/^\S+@\S+\.\S+$/.test(state.contact.email))
        stepErrors['contact.email'] = 'Email không hợp lệ';
      if (!/^(?:\+?84|0)\d{9,10}$/.test(state.contact.phone.replace(/\s|-/g, '')))
        stepErrors['contact.phone'] = 'Số điện thoại không hợp lệ';
      if (!state.acceptTerms) stepErrors.acceptTerms = 'Vui lòng đồng ý điều khoản';
    }
  ...
  function submit() {
    ...
    const payload: CreateBookingInput = {
      ...
      pickup: {
        ...state.pickup,
        scheduledFor: new Date(state.pickup.scheduledFor),
      },
      ...
      <Card>
        ...
        <CardContent className="space-y-5">
          ...
          <div className="flex justify-between pt-2">
            <Button variant="outline" onClick={prev} disabled={step === 0 || pending}>
              Quay lại
            </Button>
            {step < STEPS.length - 1 ? (
    ...
    <div className="grid gap-4 md:grid-cols-2">
      ...
      <Field id="species" label="Loài">
        <select
          ...
          onChange={(e) =>
            setState({ ...state, pet: { ...state.pet, species: e.target.value as FormState['pet']['species'] } })
          }
        >
      ...
      <Field id="sizeBand" label="Cân nặng" error={errors['pet.sizeBand']}>
        <select
          id="sizeBand"
          className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm"
          value={state.pet.sizeBand}
          onChange={(e) =>
            setState({ ...state, pet: { ...state.pet, sizeBand: e.target.value as FormState['pet']['sizeBand'] } })
          }
        >
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <Badge variant="secondary">Từ {formatVnd(s.basePriceVnd)}</Badge>
              </div>
            </button>
          );
        })}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-lg border bg-muted/30 p-4 text-sm">
        <p className="font-medium">Tóm tắt yêu cầu</p>
        <ul className="mt-2 space-y-1 text-muted-foreground">
          <li>
            Bé: <span className="text-foreground">{state.pet.name || '—'}</span> · {state.pet.species}{' '}
            · cỡ {state.pet.sizeBand}
          </li>
          <li>
            Dịch vụ:{' '}
            <span className="text-foreground">{selectedService?.nameVi ?? 'Chưa chọn'}</span>
          </li>
          <li>Thời gian: {state.pickup.scheduledFor.replace('T', ' ')}</li>
          {state.pickup.pickupRequired && (
            <li>
              Đón tại: {state.pickup.pickupAddress} ({state.pickup.pickupCity})
            </li>
          )}
        </ul>
      </div>
